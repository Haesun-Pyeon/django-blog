...
## 6. 문제 해결 및 개발 후기
- 회원가입 기능을 구현할 때 회원가입이 완료되면 자동 로그인이 되게 하려 했는데 처음에는 잘 안되었습니다.
    - 로그인을 하게 해주는 `django.contrib.auth`의 `login` 함수와 `LoginView.as_view()`를 할당한 `login` 함수의 이름이 겹쳐서 생긴 문제였습니다.
    - 함수를 불러올 때 `from django.contrib.auth import login as auth_login` 이렇게 `as auth_login`으로 이름을 바꿔주니 해결되었습니다.

- 클래스형 뷰의 UpdateView나 DeleteView에서는 템플릿페이지가 필요한데 굳이 필요가 없어 보이는 곳들도 있었습니다.
    - `GET`으로 들어올때는 템플릿에 연결해주고, `POST`일 때 기능을 수행하는 것이었습니다.
    - 댓글 수정의 경우 수정을 누르면 JavaScript DOM으로 폼을 생성해서 완료를 누르면 `POST`로 전달하게 하였습니다.
    - 회원, 글, 댓글 삭제의 경우도 `POST`방식으로 연결하여 추가 템플릿이 없어도 작동하도록 하였습니다.
    - 댓글 수정은 바로 삭제되도록 하였지만 글 삭제나 회원 탈퇴의 경우 한 번 더 되묻기 위해 부트스트랩의 모달창을 이용하였고 여기서도 `POST`방식으로 링크를 연결해서 바로 삭제될 수 있도록 하였습니다.
